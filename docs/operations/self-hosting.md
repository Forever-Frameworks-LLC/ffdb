...
curl -fsSLo /tmp/ffdb-install.sh \
  https://github.com/Forever-Frameworks-LLC/ffdb/releases/download/v0.3.5/install.sh
sudo sh /tmp/ffdb-install.sh --version 0.3.5 \
  --release-base https://github.com/Forever-Frameworks-LLC/ffdb/releases/download/v0.3.5 \
  --env-file /secure/path/ffdb.env --start --require-signature
```

Replace `0.3.5` with the announced version you reviewed. Without `--version` or
...
```sh
sudo FFDB_VERSION=0.3.5 \
  FFDB_RELEASE_BASE_URL=file:///srv/ffdb/releases/v0.3.5 \
  sh /srv/ffdb/releases/v0.3.5/install.sh \
