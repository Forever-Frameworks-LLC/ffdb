...
describe("documentation information architecture", () => {
  ...
  it("uses public scoped package identities and version-matched artifacts", () => {
    ...
    expect(clientSamples.some((sample) => sample?.code.includes("@ffdb/client@0.3.6"))).toBe(true);
    expect(client).toContain("ffdb-client-0.3.6.tgz");
    expect(cli).toContain("exact server version");
    expect(cli).toContain("@ffdb/cli@0.3.6");
  ...
  /etc/ffdb/single-host.env > /root/ffdb-bootstrap-token'`;
    ...
    for (const contract of [
      ...
      "ghcr.io/forever-frameworks-llc/ffdb-runtime:0.3.6",
      "ghcr.io/forever-frameworks-llc/ffdb-gateway:0.3.6",
      "POSTGRES_PASSWORD",
