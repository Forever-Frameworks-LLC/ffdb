...
services:
  ...
  api:
    image: ghcr.io/forever-frameworks-llc/ffdb-runtime:0.3.12
    environment:
      - FFDB_ENVIRONMENT
      - FFDB_HTTP_BIND
  ...
  gateway:
    image: ghcr.io/forever-frameworks-llc/ffdb-gateway:0.3.12
    environment:
...
sudo sh ffdb-install.sh --profile single-host --start --require-signature` },
          { label: "Reproducible exact tag", language: "sh", code: `VERSION=0.3.12
...
sudo sh ffdb-install.sh --profile single-host --start --require-signature \\
  ...
  sh ./install.sh --profile single-host --start --require-signature` },
        ...
        callout: { kind: "warning", title: "Pin production installs", body: "The latest URL follows the stable GitHub Release. The exact-tag example pins the supported 0.3.12 release for reproducible production automation; read that release's notes before installation or upgrade." },
      },
      {
...
npm install --global @ffdb/cli@0.3.12
ffdb --help` },
      },
      {
...
sudo ffdb-host status` },
        ...
        callout: { kind: "warning", title: "Release discovery is separate from installation", body: "The examples pin the supported 0.3.12 release while the latest URL tracks the stable channel. The installer verifies the signed checksum list, bundle, controller, and pinned images. Mirrors must preserve the release filenames and Sigstore bundle unchanged." },
      },
      {
...
sudo ffdb-host update-check --version 0.3.12
sudo FFDB_REQUIRE_SIGNATURE=1 ffdb-host update --version 0.3.12
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        code: { label: "Linux amd64", language: "sh", code: `VERSION=0.3.12
RELEASE_BASE="https://github.com/Forever-Frameworks-LLC/ffdb/releases/download/v$VERSION"
...
await ffdb.auth.signOut();` },
      ...
      },
      {
        ...
        code: { label: "Terminal", language: "sh", code: `VERSION=0.3.12
...
npm install --save-exact @ffdb/client@0.3.12
npm ls @ffdb/client` },
          { label: "Verified offline release artifact", language: "sh", code: `npm install --save-exact ./ffdb-client-0.3.12.tgz` },
...
# Open http://127.0.0.1:5173/app/ in a trusted browser`,
  ...
  },
  {
    ...
    codes: [
      { label: "Terminal", language: "sh", code: `npm install @ffdb/client@0.3.12` },
      { label: "src/ffdb.ts", language: "ts", code: clientSetup },
